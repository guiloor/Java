...
public class Pantalla {
	...
	public void mostrarJugador(int compensacionX, int compesacionY, Jugador jugador){
		compensacionX -= diferenciaX;
		compensacionY -= diferenciaY;

		for (int y = 0; y < jugador.obtenerSprite().obtenLado; y++) {
			int posicionY = y + compensacionY;
			for (int x = 0; x < jugador.obtnerSprite().obtenLado; x++) {
				int posicionX = x + compensacionX;
				// Si se sale de la pantalla, rompe el bucle
				if (posicionX < -jugador.obtenerPosicionX().obtenLado || posicionX >= ancho || posicionY < 0 || posicionY >= alto) {
					break;
				}
				...
				// Copia los pixeles que representan nuestro cuadro.
				pixeles[posicionX + posicionY * ancho] = jugador.obtenSprite().pixeles[x + y * jugador.obtenerSprite().obtenLado()];
			}
